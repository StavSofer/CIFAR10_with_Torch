...
function plotError(trainError, testError, title)
	...
	gnuplot.pngfigure('trainError-Final-Batch16.png')
...
function plotLoss(trainLoss, testLoss, title)
    ...
    gnuplot.pngfigure('trainLoss-Final-Batch16.png')
...
-- Low level
model:add(cudnn.SpatialConvolution(3,32,5,5,1,1,2,2)) -- 2400
...
-- Mid level
model:add(cudnn.SpatialConvolution(32,64,3,3,1,1,1,1)) -- 18432
...
-- Back to low level
model:add(cudnn.SpatialConvolution(64,32,3,3,1,1,1,1)) -- 18432
model:add(cudnn.SpatialBatchNormalization(32))
model:add(nn.ReLU(true))
-- 32*8*8
model:add(cudnn.SpatialConvolution(32,32,1,1)) -- 1024
...
model:add(nn.ReLU(true))
-- 32*8*8
model:add(cudnn.SpatialAveragePooling(5,5,2,2))
-- 32*8*8
...
for e = 1, epochs do
    ...
    if e % 5 == 0 then
        ...
        print(confusion)
        -- save the best model
        if testError[e] < minimumError then
            minimumError = testError[e]
            local name = 'finalModel-16Batch-Error' .. testError[e] .. '-Epoch' .. e
